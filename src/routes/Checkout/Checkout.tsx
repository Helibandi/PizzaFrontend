...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (parsed.first_name && parsed.last_name) {
        setFullName(`${parsed.first_name} ${parsed.last_name}`);
      }
      if (parsed.address) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData = {
        deliveryAddress: JSON.stringify(addressData), // Stringified address object
        cart: {
          items: cart.map(item => ({
            productId: parseInt(item.id), // Convert to number if needed
